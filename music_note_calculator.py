...
common_beat_time = int(one_minutes) / float(input_bpm) #指定BPMでの基本的な「1拍」の音価であり,4分音符の音価(ms)
whole_note_time = float(common_beat_time) * 4 #指定BPMでの全音符の音価(ms)
common_beat_time = int(one_minutes) / float(input_bpm) #指定BPMでの基本的な1拍の音価であり,4分音符の音価(ms)
...
#log2(x)の対数関数に"x分音符"のxの値を代入したものの。このあと小数部分は切り捨てて使う。
...
if int(note_time) <= 10:
    ...
    print( "この音符の音価は1000msです。秒針の速さと同じリズムです。\n" )
else:
    print("")
...
print( "この音符はBPM" + str(int(one_minutes)/(float(note_time)*6/4)) + "の「2拍3連」と同じ音価を持ちます。" )
print( "この音符はBPM" + str(int(one_minutes)/(float(note_time)*8/4)) + "の「8分音符」と同じ音価を持ちます。" )
